...
def load_futures_zones(file_path="futures_support_levels.txt"):
    """Dynamically parses futures levels from the local configuration file."""
    watchlist = []
    if not os.path.exists(file_path):
        with open(file_path, "w") as f:
            f.write("# symbol, planned_entry, take_profit, stop_loss\n")
        return watchlist
...
async def continuous_futures_tracker():
    ...
    try:
        while True:
            ...
            print("=" * 150)
            print(f" {BOLD_WHITE}DIRECTIVE INTRADAY FUTURES TRACKER - [{env_mode}]  (Looping Refresh: 10s){RESET}")
            print(" Press [Ctrl + C] to safely shut down the tracker window.")
            print("=" * 150)
            print(f"{BOLD_WHITE}{'Ticker':<8} | {'Future Price':<12} | {'Trade Direction':<16} | {'Planned Entry':<13} | {'Take Profit':<13} | {'Stop Loss':<11} | {'Zone Target Progress Bar & %'}{RESET}")
            print("-" * 150)
            ...
            for plan in watchlist:
                ...
                # Fetch mark price from open positions
                market_price = live_marks.get(sym, Decimal("0.00"))
                
                # Baseline fallback if position is pending fill
                if market_price == 0:
                    market_price = entry  
                ...
                # Check for active position in the account matching the contract symbol
                is_filled = any(sym in active_sym for active_sym in active_positions)
                ...
                # Double spaced, multi-column output matching the stock dashboard format
        ...
        print(f"\n\n{BOLD_WHITE}[-] Futures dashboard closed out cleanly. Returning to terminal control.{RESET}\n")

if __name__ == "__main__":
    try:
        asyncio.run(continuous_futures_tracker())
