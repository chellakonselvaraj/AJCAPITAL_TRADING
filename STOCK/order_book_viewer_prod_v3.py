...
async def main():
    ...
    def calculate_option_dte(symbol_str):
        ...
        try:
            ...
            if match:
                ...
                return exp_date.strftime('%Y-%m-%d'), str(dte)
        except Exception:
            pass
        return "--", "--"
    ...
    # Add consolidated pending orders with extracted structural dates
    for o_id, data in pending_groups.items():
        # FILTER: Ignore pending orders with an empty/zero cost basis or empty leg structure
        ...
        exp_str, dte_str = calculate_option_dte(first_leg)
        
        display_symbol = f"{base_ticker} [{len(data['legs'])} Legs]" if len(data['legs']) > 1 else first_leg
    ...
    for pos in live_positions:
        ...
        # FILTER: Skip completely closed rows or empty quantities
        if qty == 0:
            continue
        ...
        # FILTER: Skip empty cost bases on open holdings to prevent companion leg clones
        if cost_basis <= 0:
            continue

        pos_action = "Short Position" if qty < 0 else "Long Position"
        exp_str, dte_str = calculate_option_dte(symbol)
    ...
    # 4. Sort: Open static positions first, then grouped live pending components
    master_list = sorted(master_list, key=lambda x: (x['is_pending'], x['symbol']))
    ...
    # Print dynamically configured grid layout table
    print("-" * total_line_len)
    print(f"{'Order Date':<{widths['date']}} | {'Symbol / Complex':<{widths['symbol']}} | {'Action / Status':<{widths['action']}} | {'Qty':<{widths['qty']}} | {'Net Cost':<{widths['price']}} | {'Expiry':<{widths['expiry']}} | {'DTE':<{widths['dte']}} | {'Target TP (80%)':<{widths['tp']}} | {'Max SL (20%)':<{widths['sl']}}")
    print("-" * total_line_len)

    for row in rows_to_print:
        print(f"{row['date']:<{widths['date']}} | {row['symbol']:<{widths['symbol']}} | {row['action']:<{widths['action']}} | {row['qty']:<{widths['qty']}} | {row['price_str']:<{widths['price']}} | {row['expiry']:<{widths['expiry']}} | {row['dte']:<{widths['dte']}} | {row['tp']:<{widths['tp']}} | {row['sl']:<{widths['sl']}}")
