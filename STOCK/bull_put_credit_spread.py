...
def log_transaction_locally(ticket_label, symbol, qty, short_stk, long_stk, credit, risk, order_id):
    ...
    with open(log_filename, "a") as log_file:
        if not file_exists:
            ...
            log_file.write("="*120 + "\n")
            log_file.write(f"{'TIMESTAMP':<22} {'ACCOUNT TYPE':<15} {'ID':<15} {'SYM':<6} {'QTY':<4} {'STRATEGY':<10} {'NET CR':<8} {'TOTAL CR':<10} {'RISK COLLATERAL':<15}\n")
            log_file.write("-"*120 + "\n")
        ...
        log_file.write(
            f"{timestamp:<22} {account_type:<15} {order_id:<15} {symbol:<6} {qty:<4} {strategy_str:<10} ${credit:<6.2f} ${total_credit:<9.2f} ${risk:<14.2f}\n"
        )
...
def display_trading_table(title, spread_data, total_net_credit, total_risk_amount, order_id="PRE-FLIGHT REVIEW", status_code="LOCAL"):
    ...
    # BALANCED ALIGNMENT CORES FOR THE THE MATRIX COLUMNS
    headers_fmt = "{:<5} {:<16} Cast {:<18} {:<8} {:<8} {:<6} {:<8} {:<10} {:<12} {:<12} {:<14} {:<14}"
...
trade_list = load_all_option_orders()
...
for symbol_input, expiration_input, qty_input in trade_list:
    ...
    display_trading_table(f"PRE-FLIGHT REVIEW FOR {symbol_input}", bbai_bull_put_credit_spread, total_net_credit, total_risk_amount)
    ...
    try:
        response = requests.post(target_url, json=bbai_bull_put_credit_spread, headers=headers)
        ...
        display_trading_table(ticket_label, bbai_bull_put_credit_spread, total_net_credit, total_risk_amount, order_id=server_order_id, status_code=status_code_out)
